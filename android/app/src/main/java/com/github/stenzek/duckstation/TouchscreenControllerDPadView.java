...
public final class TouchscreenControllerDPadView extends View {
    ...
    public void setDefaultVisibility(boolean visibility) {
...
//    }
    ...
    private void updateControllerState() {
        ...
        boolean[] newStates = new boolean[NUM_DIRECTIONS];
        newStates[DIRECTION_UP] = (mPressed && subY == 0);
        newStates[DIRECTION_RIGHT] = (mPressed && subX == 2);
        newStates[DIRECTION_DOWN] = (mPressed && subY == 2);
        newStates[DIRECTION_LEFT] = (mPressed && subX == 0);

        AndroidHostInterface hostInterface = AndroidHostInterface.getInstance();

        for (int i = 0; i < NUM_DIRECTIONS; i++) {

            // 🔥 Detect newly pressed direction
            if (!mDirectionStates[i] && newStates[i]) {
                // Vibrate on NEW press only
                if (getContext() instanceof EmulationActivity) {
                    ((EmulationActivity) getContext()).setVibration(true);
                }
            }

            mDirectionStates[i] = newStates[i];

            if (mDirectionCodes[i] >= 0) {
                hostInterface.setControllerButtonState(
                        mControllerIndex,
                        mDirectionCodes[i],
                        mDirectionStates[i]
                );
...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final float sizeMultiplier = 1.50f; // 1.0 = original size
        final int buttonWidth = (int)((width / 3f) * sizeMultiplier);
        final int buttonHeight = (int)((height / 3f) * sizeMultiplier);


        final int centerX = width / 2;
        final int centerY = height / 2;

        // This controls how close they are to center
        final float inwardFactor = 0.65f; // smaller = tighter (try 0.6 - 0.75)

        int offsetX = (int) (buttonWidth * inwardFactor);
        int offsetY = (int) (buttonHeight * inwardFactor);

        // UP
        drawAt(canvas, DIRECTION_UP,
                centerX - buttonWidth / 2,
                centerY - offsetY - buttonHeight / 2,
                buttonWidth,
                buttonHeight);

        // RIGHT
        drawAt(canvas, DIRECTION_RIGHT,
                centerX + offsetX - buttonWidth / 2,
                centerY - buttonHeight / 2,
                buttonWidth,
                buttonHeight);

        // DOWN
        drawAt(canvas, DIRECTION_DOWN,
                centerX - buttonWidth / 2,
                centerY + offsetY - buttonHeight / 2,
                buttonWidth,
                buttonHeight);
    ...
    private void drawAt(Canvas canvas, int direction,
                        int left, int top,
        ...
        final Drawable drawable = mDirectionStates[direction]
                ? mPressedDrawables[direction]
                : mUnpressedDrawables[direction];
