...
void Settings::Load(SettingsInterface& si)
{
  ...
  start_fullscreen = si.GetBoolValue("Main", "StartFullscreen", true);
  ...
  gpu_resolution_scale = static_cast<u32>(si.GetIntValue("GPU", "ResolutionScale", 8));
  ...
  gpu_24bit_chroma_smoothing = si.GetBoolValue("GPU", "ChromaSmoothing24Bit", true);
  gpu_pgxp_enable = si.GetBoolValue("GPU", "PGXPEnable", true);
  ...
  gpu_pgxp_vertex_cache = si.GetBoolValue("GPU", "PGXPVertexCache", true);
  ...
  display_post_processing = si.GetBoolValue("Display", "PostProcessing", true);
